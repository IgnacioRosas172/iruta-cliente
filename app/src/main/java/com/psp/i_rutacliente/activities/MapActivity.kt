...
import com.psp.i_rutacliente.databinding.ActivityMapBinding
import com.psp.i_rutacliente.fragments.ModalBottomSheetMenu
import com.psp.i_rutacliente.models.DriverLocation
import com.psp.i_rutacliente.providers.GeoProvider
import com.psp.i_rutacliente.utils.CarMoveAnim
...
class MapActivity : AppCompatActivity(), OnMapReadyCallback, Listener {
    ...
    private val authProvider = com.psp.i_rutacliente.providers.AuthProvider()
...
//se crea un marcador para el conductor que este conectado
                val driverLatLng = LatLng(location.latitude, location.longitude)
                val marker = googleMap?.addMarker(
                    MarkerOptions().position(driverLatLng).title("Autobus disponible").icon(
                        BitmapDescriptorFactory.fromResource(R.drawable.interbus)
                    )
                )
            ...
            override fun onKeyMoved(documentID: String, location: GeoPoint) {
                for (marker in driversMarkers) {
                    ...
                    val start = LatLng(location.latitude, location.longitude) //se captura la posicion inicial
                    var end: LatLng? = null //posicion final
    ...
    private fun goToTripInfo() {
        if (originLatLng != null && destinationLatLng != null) {
            ...
            Toast.makeText(this, "Debes de seleccionar el origen y el destino", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun onCameraMove() {
        googleMap?.setOnCameraIdleListener {
            try {
                ...
                originLatLng = googleMap?.cameraPosition?.target //devuelve una clase latlong
    ...
    private fun instanceAutocompleteOrigin() {
        ...
        autoCompleteOrigin?.setOnPlaceSelectedListener(object: PlaceSelectionListener {
            ...
            override fun onError(p0: Status) {//override del object
